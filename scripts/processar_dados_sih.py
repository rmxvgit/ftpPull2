...
def CalculaSelic(data_inicio: dict[str, int], data_fim: dict[str, int], selic_array: list[float]):
    ...
    posicao_do_fim_na_array = (data_fim['year'] - inicio_da_selic['year'])*12 + data_fim['month'] - inicio_da_selic['month']
...
def correcao_absoluta(data_inicio: dict[str, int], data_fim: dict[str, int], selic_arr: list[float]):
...
def processar_dados_csv(csv_file_path: str, output_file_path: str, data_inicio: dict[str, int], data_fim: dict[str, int]):
    ...
    df_desc['SP_VALATO'] = df_desc['PA_VALPRO'].map(lambda x: f"{x:,.2f}".replace('.', ','))
